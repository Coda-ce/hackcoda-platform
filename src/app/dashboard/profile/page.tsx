...
export default function ProfilePage() {
  ...
  const handleSave = () => {
    setIsEditing(false);
    // TODO: Adicionar chamada a API para atualizar perfil
  };
  ...
  return (
    <main className="flex items-center justify-center">
      <Card className="w-full z-10 border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        <CardHeader className="flex flex-row justify-between items-center py-8 mb-6">
          <div>
            <CardTitle className="text-3xl font-black tracking-tight text-white">
              Meu Perfil
            </CardTitle>
            <CardDescription className="text-brasil-verdeEscuro uppercase tracking-widest text-sm font-bold mt-1">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="group space-y-2">
              <label className="text-sm font-medium text-zinc-300 ml-1 flex items-center gap-2 group-focus-within:text-brasil-verde group-focus-within:font-bold transition-all duration-100">
          ...
          <div className="space-y-3 p-4 bg-brasil-verde/10 border border-brasil-verde/20 rounded-lg">
            <p className="text-sm text-zinc-300">
              <span className="font-medium text-brasil-verde">Dica:</span>{" "}
